...
class GameObject {
  constructor(config) {
    ...
    this.stepsTilTarget =
      config.stepsTilTarget === undefined ? 24 : config.stepsTilTarget;
    ...
    this.behaviours = {
      [BEHAVIOUR.IDLE]: [
        ...
        // could use small time value to have them run around chaotically!
        {
          type: "stand",
          direction: "left",
          time: Math.random() * this.standTime,
        },
      ...
      [BEHAVIOUR.TALK]: [{ type: "talking" }],
    };

    // https://www.youtube.com/watch?v=e144CXGy2mc part 8
    this.behaviourLoop =
      config.behaviourLoop || this.behaviours[BEHAVIOUR.IDLE];
  ...
  update() {
    ...
    if (
      this.behaviourLoop[this.behaviourLoopIndex].type === "walk" &&
      this.stepsTilTarget > 0
    ) {
      this.updatePosition();
  ...
  async doBehaviorEvent() {
    ...
    if (this.behaviourLoop.length === 0 /* || !this.sprite.isLoaded */) {
